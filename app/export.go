...
func (app *App) prepForZeroHeightGenesis(ctx sdk.Context, jailAllowedAddrs []string) {
	...
	for _, addr := range jailAllowedAddrs {
		...
		if err != nil {
			// checked: valid use of log fatal
			log.Fatal(err)
		}
	...
	err := app.StakingKeeper.IterateValidators(ctx, func(_ int64, val stakingtypes.ValidatorI) (stop bool) {
		...
		// Try to withdraw validator commission with proper error handling
	...
	for _, delegation := range dels {
		...
		// Try to withdraw delegation rewards with proper error handling
	...
	err = app.StakingKeeper.IterateValidators(ctx, func(_ int64, val stakingtypes.ValidatorI) (stop bool) {
		...
		// Get any remaining outstanding rewards
		scraps, err := app.DistrKeeper.GetValidatorOutstandingRewardsCoins(ctx, valBz)
		if err != nil {
			panic(err)
		}

		// Only donate to community pool if there are actually outstanding rewards
		// This makes the donation more explicit and traceable
		if !scraps.IsZero() {
			app.Logger().Debug("donating outstanding rewards to community pool",
				"validator", val.GetOperator(),
				"amount", scraps.String())
	...
	// Log final summary of the zero-height genesis preparation
	app.Logger().Info("completed zero-height genesis preparation")
}
