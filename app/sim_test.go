...
const (
	SimAppChainID = "zigchain-simapp"
)

// MakeTestTxConfig creates a TxConfig with the correct bech32 prefixes for simulation
func MakeTestTxConfig() client.TxConfig {
	interfaceRegistry := testutil.CodecOptions{
		AccAddressPrefix: constants.AddressPrefix,
		ValAddressPrefix: constants.AddressPrefix + "valoper",
	}.NewInterfaceRegistry()
	cdc := codec.NewProtoCodec(interfaceRegistry)
	return tx.NewTxConfig(cdc, tx.DefaultSignModes)
}

// BuildSimulationOperations builds simulation operations with the correct TxConfig
// that has the proper bech32 prefixes configured
func BuildSimulationOperations(app runtime.AppI, cdc codec.JSONCodec, config simulationtypes.Config, txConfig client.TxConfig) []simulationtypes.WeightedOperation {
	simState := module.SimulationState{
		AppParams: make(simulationtypes.AppParams),
		Cdc:       cdc,
		TxConfig:  txConfig,
		BondDenom: constants.BondDenom,
	}
...
func BenchmarkSimulation(b *testing.B) {
	...
	// Check if there is an error, if there is, fail the test
	require.NoError(b, err, "simulation setup failed")

	// On cleanup, close the database and remove the directory
	defer func() {
		// Close the database
		require.NoError(b, db.Close())
		// Remove the directory that was created for the simulation
		require.NoError(b, os.RemoveAll(dir))
	}()

	// Create a new application options map
	appOptions := make(simtestutil.AppOptionsMap, 0)
	// Set the home flag to the default node home, this is usually set to ~/.zigchain
	appOptions[flags.FlagHome] = app.DefaultNodeHome
	// Set the inv check period flag to the flag period value
	// run slow invariants only once every period assertions (default 0, never)
	...
	// Check if the application name is equal to the expected application name
	...
	_, simParams, simErr := simulation.SimulateFromSeed(
		...
		BuildSimulationOperations(bApp, bApp.AppCodec().(codec.JSONCodec), config, MakeTestTxConfig()),
		keepers.BlockedAddresses(),
		config,
		bApp.AppCodec(),
	)
