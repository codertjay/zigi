...
func initAppForTestnet(app *app.App, args valArgs) *app.App {
	...
	// #nosec G104 -- This is a test file, error handling is not critical for iterator closure
	iterator.Close()
	...
	// #nosec G104 -- This is a test file, error handling is not critical for validator setting
	app.StakingKeeper.SetValidator(ctx, newVal)
	err = app.StakingKeeper.SetValidatorByConsAddr(ctx, newVal)
	if err != nil {
		tmos.Exit(err.Error())
	}
	// #nosec G104 -- This is a test file, error handling is not critical for validator power index
	app.StakingKeeper.SetValidatorByPowerIndex(ctx, newVal)
	// #nosec G104 -- This is a test file, error handling is not critical for validator power
	...
	// #nosec G104 -- This is a test file, error handling is not critical for validator rewards
	...
	// #nosec G104 -- This is a test file, error handling is not critical for validator signing info
	...
	params.MinDeposit = sdk.NewCoins(sdk.NewInt64Coin(bondDenom, 100))
	params.ExpeditedMinDeposit = sdk.NewCoins(sdk.NewInt64Coin(bondDenom, 10000))
	expeditedVotingPeriod := time.Second * 120
	params.ExpeditedVotingPeriod = &expeditedVotingPeriod
	// #nosec G104 -- testnet setup, error handling not critical
