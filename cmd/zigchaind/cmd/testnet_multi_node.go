...
Example:
	zigchaind multi-node --v 4 --output-dir ./.testnets --validators-stake-amount 1000000,200000,300000,400000 --list-ports 47222,50434,52851,44210
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if s, err := cmd.Flags().GetString(flagValidatorsStakeAmount); err == nil {
				for _, amount := range strings.Split(s, ",") {
					...
					args.validatorsStakesAmount[top] = sdk.NewCoin(constants.BondDenom, a)
					top += 1
				}
...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0%s", constants.BondDenom), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.001uzig)")
...
func initTestnetFiles(
	...
	appConfig.BaseConfig.MinGasPrices = "0" + constants.BondDenom
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			...
			sdk.NewCoin(constants.BondDenom, accStakingTokens),
		}
		...
		if !ok {
			valTokens = sdk.NewCoin(constants.BondDenom, sdk.TokensFromConsensusPower(100, sdk.DefaultPowerReduction))
		}
	...
	for i := 0; i < args.numValidators; i++ {
		for _, file := range gentxsFiles {
			...
			// #nosec G104 -- This is a test file, error handling is not critical for file copying
...
func writeFile(file, dir string, contents []byte) error {
	// #nosec G301 -- This is a test file, directory permissions are acceptable for test environment
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return fmt.Errorf("could not create directory %q: %w", dir, err)
	}

	// #nosec G306 -- This is a test file, file permissions are acceptable for test environment
...
func initGenFiles(
	...
	// set staking module parameters to use uzig as bond denomination
	var stakingGenState stakingtypes.GenesisState
	clientCtx.Codec.MustUnmarshalJSON(appGenState[stakingtypes.ModuleName], &stakingGenState)
	stakingGenState.Params.BondDenom = constants.BondDenom
	appGenState[stakingtypes.ModuleName] = clientCtx.Codec.MustMarshalJSON(&stakingGenState)

	// set mint module parameters to use uzig as mint denomination
	var mintGenState minttypes.GenesisState
	clientCtx.Codec.MustUnmarshalJSON(appGenState[minttypes.ModuleName], &mintGenState)
	mintGenState.Params.MintDenom = constants.BondDenom
	appGenState[minttypes.ModuleName] = clientCtx.Codec.MustMarshalJSON(&mintGenState)

	var govGenState govtypesv1.GenesisState
	clientCtx.Codec.MustUnmarshalJSON(appGenState[govtypes.ModuleName], &govGenState)
	govGenState.Params.MinDeposit = sdk.NewCoins(sdk.NewCoin(constants.BondDenom, math.NewInt(100)))
	govGenState.Params.ExpeditedMinDeposit = sdk.NewCoins(sdk.NewCoin(constants.BondDenom, math.NewInt(1000)))
	expeditedVotingPeriod := time.Second * 60
...
func copyFile(src, dstDir string) (int64, error) {
	...
	// #nosec G304 -- This is a test file, file path is controlled and safe
...
func generateRandomString(length int) string {
	...
	// #nosec G404 -- This is a test file, weak random number generator is acceptable
